...
// BitGold: get current stake weight
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, CKey& key)
{
    ...
    int64 nCombineThreshold = GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, (GetLastBlockIndex(pindexBest, false)->nHeight)+1) / 3;
