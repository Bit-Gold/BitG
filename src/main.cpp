...
const string strMessageMagic = "BitGold Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // BitGold: enforce minimum output amount for user transactions until 1 May 2014 04:00:00 GMT
...
int64 GetProofOfWorkReward(unsigned int nBits, int nHeight)
{
		...
		if(nHeight < 1001) 
			nSubsidy = 20 * COIN;								
	    else if(nHeight > 601001 && nHeight < 1401001)   
			nSubsidy = 0.25 * COIN;
		else if(nHeight > 1401000)
			nSubsidy = 0 * COIN;
...
{
    int64 nRewardCoinYear = 10 * CENT;	// Year 1
	
	if(nTime > YEARTWO)					// Year 2
		nRewardCoinYear = 7.5 * CENT;
	if(nTime > YEARTHREE)				// Year 3
		nRewardCoinYear = 5.0 * CENT;
	if(nTime > YEARFOUR)				// Year 4
		nRewardCoinYear = 3.0 * CENT;
	if(nTime > YEARFIVE)				// Year 5
		nRewardCoinYear = 1.0 * CENT;
...
static const int64 nTargetTimespan = 2 * 24 * 60 * 60;  // 2 days
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BitGold
    bool fStrictPayToScriptHash = true; // Always active in BitGold
	...
	// Check that coinbase reward doesn't exceed maximum
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // BitGold: check proof-of-stake block signature
        if (fCheckSig && !CheckBlockSignature(true))
            return DoS(100, error("CheckBlock() : bad proof-of-stake block signature"));
    }
    else
    {
        int64 nReward = GetProofOfWorkReward(nBits, 1);
        ...
        if(!fTestNet && fCheckSig)
        {
            ...
            // BitGold: check proof-of-work block signature
...
// BitGold: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitGold", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BitGold!";
        CTransaction txNew;
        txNew.nTime = 1395864533;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
++block.nTime;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xaa5cbc62a9fcae81bbd28c7c723dccdc628a5a68a8ff8c7f5b9e6627427533d8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe3, 0xd8, 0xf9, 0xc5 };
